 ...
 * </p>
 * <p>
 * This implementation is optimised assuming that the timeout will mostly be
 * cancelled and then reused with a similar value.
 * </p>
 * <p>
 * The typical scenario to use this class is when you have events that postpone
 * (by re-scheduling), or cancel then re-schedule, a timeout for a single
 * entity. For example: connection idleness, where for each connection there is
 * a CyclicTimeout and a read/write postpones the timeout; when the timeout
 * expires, the implementation checks against a timestamp if the connection is
 * really idle. Another example: HTTP session expiration, where for each HTTP
 * session there is a CyclicTimeout and at the beginning of the request
 * processing the timeout is canceled (via cancel()), but at the end of the
 * request processing the timeout is re-scheduled.
 * </p>
 * <p>
 * This implementation has a {@link Timeout} holding the time at which the
 * scheduled task should fire, and a linked list of {@link Wakeup}, each holding
 * the actual scheduled task.
 * </p>
 * <p>
 * Calling {@link #schedule(long, TimeUnit)} the first time will create a
 * Timeout with an associated Wakeup and submit a task to the scheduler. Calling
 * {@link #schedule(long, TimeUnit)} again with the same or a larger delay will
 * cancel the previous Timeout, but keep the previous Wakeup without submitting
 * a new task to the scheduler, therefore reducing the pressure on the scheduler
 * and avoid it becomes a bottleneck. When the Wakeup task fires, it will see
 * that the Timeout is now in the future and will attach a new Wakeup with the
 * future time to the Timeout, and submit a scheduler task for the new Wakeup.
 * </p>
 *
 * @see CyclicTimeouts
 */
public abstract class CyclicTimeout implements Destroyable {
	 ...
	 * </p>
	 * <p>
	 * If a timeout is already set, it will be cancelled and replaced by the new
	 * one.
	 * </p>
	 *
		...
		while (true) {
			...
			if (_timeout.compareAndSet(timeout, new Timeout(newTimeoutAt, wakeup))) {
				if (LOG.isDebugEnabled()) {
					LOG.debug("Installed timeout in {} ms, {} wake up in {} ms", units.toMillis(delay),
							newWakeup != null ? "new" : "existing", TimeUnit.NANOSECONDS.toMillis(wakeup._at - now));
				}
				break;
			}
		}
	...
	private static class Timeout {
		...
		public String toString() {
			return String.format("%s@%x:%dms,%s", getClass().getSimpleName(), hashCode(),
					TimeUnit.NANOSECONDS.toMillis(_at - System.nanoTime()), _wakeup);
		}
	}
	...
	private class Wakeup implements Runnable {
		...
		public void run() {
			...
			while (true) {
				...
				// wakeup list, effectively removing any wakeup before us in the list (and
				// making them no-ops).
				// If we don't find ourselves, then a wakeup that should have expired after us
				// has already run
		...
		public String toString() {
			return String.format("%s@%x:%dms->%s", getClass().getSimpleName(), hashCode(),
					_at == MAX_VALUE ? _at : TimeUnit.NANOSECONDS.toMillis(_at - System.nanoTime()), _next);
		}
	}
}