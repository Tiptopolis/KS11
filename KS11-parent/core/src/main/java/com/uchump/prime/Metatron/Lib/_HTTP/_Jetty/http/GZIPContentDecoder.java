 ...
 * </p>
 * <p>
 * This decoder inflates gzip compressed data, and has been optimized for async
 * usage with minimal data copies.
 * </p>
 */
public class GZIPContentDecoder implements Destroyable {
	 ...
	 * {@link #release(ByteBuffer)}.
	 * </p>
	 * <p>
	 * This method may fully consume the input buffer, but return only a chunk of
	 * the inflated bytes, to allow applications to consume the inflated chunk
	 * before performing further inflation, applying backpressure. In this case,
	 * this method should be invoked again with the same input buffer (even if it's
	 * already fully consumed) and that will produce another chunk of inflated
	 * bytes. Termination happens when the input buffer is fully consumed, and the
	 * returned buffer is empty.
	 * </p>
	 * <p>
	 * See {@link #decodedChunk(ByteBuffer)} to perform inflating in a non-blocking
	 * way that allows to apply backpressure.
	 * </p>
	 *
	 ...
	 * </p>
	 * <p>
	 * The default implementation aggregates all the chunks into a single buffer
	 * returned from {@link #decode(ByteBuffer)}.
	 * </p>
	 * <p>
	 * Derived implementations may choose to consume inflated chunks individually
	 * and return {@code true} from this method to prevent further inflation until a
	 * subsequent call to {@link #decode(ByteBuffer)} or
	 * {@link #decodeChunks(ByteBuffer)} is made.
	 *
	 * @param chunk the inflated chunk of data
	 * @return false if inflating should continue, or true if the call to
	 *         {@link #decodeChunks(ByteBuffer)} or {@link #decode(ByteBuffer)}
	 ...
	 * more compressed data or a call to {@link #decodedChunk(ByteBuffer)} returns
	 * true.
	 * </p>
	 *
	 ...
	 * @return a heap buffer of the configured capacity either from the pool or
	 *         freshly allocated.
	 */
	 ...
	 * </p>
	 * <p>
	 * This method calls {@link ByteBufferPool#release(ByteBuffer)} if a buffer pool
	 * has been configured.
	 * </p>
	 * <p>
	 * This method should be called once for all buffers returned from
	 * {@link #decode(ByteBuffer)} or passed to {@link #decodedChunk(ByteBuffer)}.
	 * </p>
	 *
