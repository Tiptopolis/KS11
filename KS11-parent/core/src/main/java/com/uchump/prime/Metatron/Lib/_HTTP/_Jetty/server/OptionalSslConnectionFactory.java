...
/**
 * <p>
 * A ConnectionFactory whose connections detect whether the first bytes are TLS
 * bytes and upgrades to either a TLS connection or to another configurable
 * connection.
 * </p>
 *
 * @deprecated Use {@link DetectorConnectionFactory} with a
 *             {@link SslConnectionFactory} instead.
 */
@Deprecated
public class OptionalSslConnectionFactory extends DetectorConnectionFactory {
	...
	/**
	 * <p>
	 * Creates a new ConnectionFactory whose connections can upgrade to TLS or
	 * another protocol.
	 * </p>
	 *
	 * @param sslConnectionFactory The {@link SslConnectionFactory} to use if the
	 *                             first bytes are TLS
	 * @param nextProtocol         the protocol of the {@link ConnectionFactory} to
	 *                             use if the first bytes are not TLS, or null to
	 *                             explicitly handle the non-TLS case
	 */
	 ...
	 * </p>
	 * <p>
	 * This typically happens when a client is trying to connect to a TLS port using
	 * the {@code http} scheme (and not the {@code https} scheme).
	 * </p>
	 *
	...
	protected void nextProtocol(Connector connector, EndPoint endPoint, ByteBuffer buffer) {
		...
		if (_nextProtocol != null) {
			...
			if (connectionFactory == null)
				throw new IllegalStateException("Cannot find protocol '" + _nextProtocol
						+ "' in connector's protocol list " + connector.getProtocols() + " for " + endPoint);
	...
	/**
	 * <p>
	 * Legacy callback method invoked when {@code nextProtocol} is {@code null} and
	 * the first bytes are not TLS.
	 * </p>
	 * <p>
	 * This typically happens when a client is trying to connect to a TLS port using
	 * the {@code http} scheme (and not the {@code https} scheme).
	 ...
	 * @param buffer   The buffer with the first bytes of the connection
	 * @param endPoint The connection EndPoint object
	 * @deprecated Override {@link #nextProtocol(Connector, EndPoint, ByteBuffer)}
	 *             instead.
	 */
	@Deprecated
