...
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.client.Synchronizable;
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.client.api.Request.Listener;
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.util.BufferUtil;
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.util.Callback;

/**
 * A {@link ContentProvider} that allows to add content after
 * {@link Request#send(Response.CompleteListener)} has been called, therefore
 * providing the request content at a later time.
 * <p>
 * {@link DeferredContentProvider} can only be used in conjunction with
 * {@link Request#send(Response.CompleteListener)} (and not with its blocking
 * counterpart {@link Request#send()}) because it provides content
 * asynchronously.
 * <p>
 * The deferred content is provided once and then fully consumed. Invocations to
 ...
 * However, it is possible for subclasses to override {@link #offer(ByteBuffer)}
 * and {@link #close()} to copy the content to another location (for example a
 * file) and be able to support multiple invocations of of {@link #iterator()}
 * returning the iterator provided by this class on the first invocation, and an
 * iterator on the bytes copied to the other location for subsequent
 * invocations.
 * <p>
 * Typical usage of {@link DeferredContentProvider} is in asynchronous proxies,
 * where HTTP headers arrive separately from HTTP content chunks.
 * <p>
 * The deferred content must be provided through {@link #offer(ByteBuffer)},
 * which can be invoked multiple times, and when all content has been provided
 * it must be signaled with a call to {@link #close()}.
 * <p>
 * Example usage:
 * 
 * <pre>
...
public class DeferredContentProvider implements AsyncContentProvider, Callback, Closeable {
	...
	/**
	 * Adds the given content buffer to this content provider and notifies the
	 * listener that content is available.
	 *
	...
	/**
	 * No more content will be added to this content provider and notifies the
	 * listener that no more content is available.
	 */
	@Override
	public void close() {
