 ...
 * </p>
 * <p>
 * Calculates the rate at which the {@link #record()} method is called over the
 * configured period, retaining also the total count and maximum rate achieved.
 * </p>
 * <p>
 * The implementation keeps a Deque of timestamps for all records for the last
 * time period, so this method is not suitable for large rates unless a small
 * time period is used.
 * </p>
 */
public class RateStatistic {
	...
	public String dump(TimeUnit units) {
		long now = System.nanoTime();
		try (AutoLock l = _lock.lock()) {
			String samples = _samples.stream().mapToLong(t -> units.convert(now - t, TimeUnit.NANOSECONDS))
	...
	private String toString(long nanoTime) {
		try (AutoLock l = _lock.lock()) {
			update(nanoTime);
			return String.format("%s@%x{count=%d,max=%d,rate=%d per %d %s}", getClass().getSimpleName(), hashCode(),
					_count, _max, _samples.size(), _units.convert(_nanoPeriod, TimeUnit.NANOSECONDS), _units);
		}
	}
}