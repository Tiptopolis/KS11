 ...
 * {@link EndPoint#write(Callback, ByteBuffer...)} by calling
 * {@link EndPoint#flush(ByteBuffer...)} until all content is written. The
 * abstract method {@link #onIncompleteFlush()} is called when not all content
 * has been written after a call to flush and should organize for the
 * {@link #completeWrite()} method to be called when a subsequent call to flush
...
public abstract class WriteFlusher {
	...
	private static final boolean DEBUG = LOG.isDebugEnabled(); // Easy for the compiler to remove the code if
																// DEBUG==false
	...
	//
	// If a failure happens while in IDLE, it is a noop since there is no operation
	// to tell of the failure.
	// IDLE--(fail)-->IDLE
	//
	// From any other state than IDLE a failure will result in an FAILED state which
	 ...
	 * @throws IllegalStateException if previous to next is not a legal state
	 *                               transition (api usage error)
	 */
	...
	/**
	 * In FailedState no more operations are allowed. The current implementation
	 * will never recover from this state.
	 */
	...
	/**
	 * In CompletingState WriteFlusher is flushing buffers that have not been fully
	 * written in write(). If write() didn't flush all buffers in one go, it'll
	 * switch the State to PendingState. completeWrite() will then switch to this
	 * state and try to flush the remaining buffers.
	 */
	...
	/**
	 * In PendingState not all buffers could be written in one go. Then write() will
	 * switch to PendingState() and preserve the state by creating a new
	 * PendingState object with the given parameters.
	 */
	...
	public InvocationType getCallbackInvocationType() {
		State s = _state.get();
		return (s instanceof PendingState) ? ((PendingState) s).getCallbackInvocationType()
				: Invocable.InvocationType.BLOCKING;
	}

	/**
	 * Abstract call to be implemented by specific WriteFlushers. It should schedule
	 * a call to {@link #completeWrite()} or {@link #onFail(Throwable)} when
	 * appropriate.
	 */
	protected abstract void onIncompleteFlush();

	/**
	 * Tries to switch state to WRITING. If successful it writes the given buffers
	 * to the EndPoint. If state transition fails it will fail the callback and
	 * leave the WriteFlusher in a terminal FAILED state.
	 *
	 * If not all buffers can be written in one go it creates a new
	 * {@code PendingState} object to preserve the state and then calls
	 * {@link #onIncompleteFlush()}. The remaining buffers will be written in
	 * {@link #completeWrite()}.
	 *
	 ...
	 * {@link #onIncompleteFlush()} to request a call to this method when a call to
	 * {@link EndPoint#flush(ByteBuffer...)} is likely to be able to progress.
	 *
	 * It tries to switch from PENDING to COMPLETING. If state transition fails,
	 * then it does nothing as the callback should have been already failed. That's
	 * because the only way to switch from PENDING outside this method is
	 ...
	 *
	 * @param address the datagram channel to send the buffers to (used by QUIC and
	 *                HTTP/3)
	...
	protected ByteBuffer[] flush(SocketAddress address, ByteBuffer[] buffers) throws IOException {
		...
		while (progress && buffers != null) {
			...
			boolean flushed = address == null ? _endPoint.flush(buffers)
					: ((DatagramChannelEndPoint) _endPoint).send(address, buffers);
		...
		// If buffers is null, then flush has returned false but has consumed all the
		// data!
		// This is probably SSL being unable to flush the encrypted buffer, so return
		// EMPTY_BUFFERS
	...
	public String toString() {
		State s = _state.get();
		return String.format("WriteFlusher@%x{%s}->%s", hashCode(), s,
				s instanceof PendingState ? ((PendingState) s)._callback : null);
	}

	/**
	 * <p>
	 * A listener of {@link WriteFlusher} events. If implemented by a Connection
	 * class, the {@link #onFlushed(long)} event will be delivered to it.
	 * </p>
	 */
	public interface Listener {
		/**
		 * <p>
		 * Invoked when a {@link WriteFlusher} flushed bytes in a non-blocking way, as
		 * part of a - possibly larger - write.
		 * </p>
		 * <p>
