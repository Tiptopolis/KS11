    ...
    public void handleField(Class<?> clazz, Field field)
    {
        ...
        if (resource != null)
        {
            ...
            if (injection == null)
            {
                ...
                try
                {
                    boolean bound = com.uchump.prime.Metatron.Lib._HTTP._Jetty.plus.jndi.NamingEntryUtil.bindToENC(_context, name, mappedName);
                    if (!bound)
                        bound = com.uchump.prime.Metatron.Lib._HTTP._Jetty.plus.jndi.NamingEntryUtil.bindToENC(_context.getServer(), name, mappedName);
                    if (!bound)
                        bound = com.uchump.prime.Metatron.Lib._HTTP._Jetty.plus.jndi.NamingEntryUtil.bindToENC(null, name, mappedName);
                    if (!bound)
                    {
