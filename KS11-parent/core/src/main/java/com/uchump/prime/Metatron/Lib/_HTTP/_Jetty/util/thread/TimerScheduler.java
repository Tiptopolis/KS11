...
public class TimerScheduler extends AbstractLifeCycle implements Scheduler, Runnable {
	...
	/*
	 * This class uses the Timer class rather than an ScheduledExecutionService
	 * because it uses the same algorithm internally and the signature is cheaper to
	 * use as there are no Futures involved (which we do not need). However, Timer
	 * is still locking and a concurrent queue would be better.
	 */
