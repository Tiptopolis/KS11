 ...
 * <p>
 * Applications can disable redirection via
 * {@link Request#followRedirects(boolean)} and then rely on this class to
 * perform the redirect in a simpler way, for example:
 * 
 * <pre>
...
public class HttpRedirector {
	...
	/**
	 * Redirects the given {@code response}, blocking until the redirect is
	 * complete.
	 *
	 * @param request  the original request that triggered the redirect
	 * @param response the response to the original request
	 * @return a {@link Result} object containing the request to the redirected
	 *         location and its response
	...
	public Result redirect(Request request, Response response) throws InterruptedException, ExecutionException {
		...
		Request redirect = redirect(request, response, new BufferingResponseListener() {
			...
			public void onComplete(Result result) {
				resultRef.set(new Result(result.getRequest(), result.getRequestFailure(),
	...
	private Request redirect(Request request, Response response, Response.CompleteListener listener, URI newURI) {
		...
		case 301: {
			...
			fail(request, response, new HttpResponseException(
	...
	private Request redirect(Request request, Response response, Response.CompleteListener listener, URI location,
			String method) {
	...
	/**
	 * Extracts and sanitizes (by making it absolute and escaping paths and query
	 * parameters) the redirect URI of the given {@code response}.
	 *
	 * @param response the response to extract the redirect URI from
	 * @return the absolute redirect URI, or null if the response does not contain a
	 *         valid redirect location
	 */
	...
	private URI sanitize(String location) {
		// Redirects should be valid, absolute, URIs, with properly escaped paths and
		// encoded
	...
	private Request sendRedirect(HttpRequest httpRequest, Response response, Response.CompleteListener listener,
			URI location, String method) {
		try {
