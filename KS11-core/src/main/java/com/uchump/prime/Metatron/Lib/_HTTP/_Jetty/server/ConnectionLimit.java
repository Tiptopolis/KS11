...
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.io.SelectorManager;
 ...
 * to prevent further connections being received. It can be applied to an entire
 * server or to a specific connector by adding it via
 * {@link Container#addBean(Object)}
 * </p>
 * <p>
 * <b>Usage:</b>
 * </p>
...
public class ConnectionLimit extends AbstractLifeCycle implements Listener, SelectorManager.AcceptListener {
	...
	/**
	 * @return If &gt;= 0, the endpoint idle timeout in ms to apply when the
	...
	/**
	 * @param idleTimeout If &gt;= 0 the endpoint idle timeout in ms to apply when
	...
	public void onAcceptFailed(SelectableChannel channel, Throwable cause) {
		try (AutoLock l = _lock.lock()) {
			...
			if (LOG.isDebugEnabled())
				LOG.debug("onAcceptFailed ({}+{}) < {} {} {}", _accepting.size(), _connections, _maxConnections,
						channel, cause);
			check();
		}
	}
