...
/**
 * <p>
 * Partial implementation of EndPoint that uses {@link FillInterest} and
 * {@link WriteFlusher}.
 * </p>
 */
public abstract class AbstractEndPoint extends IdleTimeout implements EndPoint {
	...
	protected void onIdleExpired(TimeoutException timeout) {
		...
		// If the endpoint is half closed and there was no fill/write handling, then
		// close here.
		// This handles the situation where the connection has completed its close
		// handling
		// and the endpoint is half closed, but the other party does not complete the
		// close.
		// This perhaps should not check for half closed, however the servlet spec case
		// allows
		// for a dispatched servlet or suspended request to extend beyond the
		// connections idle
		// time. So if this test would always close an idle endpoint that is not
		// handled, then
	...
	public void upgrade(Connection newConnection) {
		...
		if (LOG.isDebugEnabled())
			LOG.debug("{} upgrading from {} to {} with {}", this, oldConnection, newConnection,
		...
		if (BufferUtil.hasContent(buffer)) {
			if (newConnection instanceof Connection.UpgradeTo)
				...
				throw new IllegalStateException("Cannot upgrade: " + newConnection + " does not implement "
						+ Connection.UpgradeTo.class.getName());
		}
	...
	public String toString() {
		return String.format("%s@%x[%s]->[%s]", getClass().getSimpleName(), hashCode(), toEndPointString(),
				toConnectionString());
	}

	public String toEndPointString() {
		return String.format("{l=%s,r=%s,%s,fill=%s,flush=%s,to=%d/%d}", getLocalSocketAddress(),
				getRemoteSocketAddress(), _state.get(), _fillInterest.toStateString(), _writeFlusher.toStateString(),
				getIdleFor(), getIdleTimeout());
	}
