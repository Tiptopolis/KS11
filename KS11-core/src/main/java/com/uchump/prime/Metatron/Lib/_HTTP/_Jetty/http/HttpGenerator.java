...
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.http.HttpTokens.EndOfContent;
...
import static com.uchump.prime.Metatron.Lib._HTTP._Jetty.http.HttpStatus.INTERNAL_SERVER_ERROR_500;

/**
 * HttpGenerator. Builds HTTP Messages.
 * <p>
 * If the system property "org.eclipse.jetty.http.HttpGenerator.STRICT" is set
...
public class HttpGenerator {
	...
	public static final MetaData.Response PROGRESS_102_INFO = new MetaData.Response(HttpVersion.HTTP_1_1, 102, null,
			null, -1);
	public static final MetaData.Response RESPONSE_500_INFO = new MetaData.Response(HttpVersion.HTTP_1_1,
			INTERNAL_SERVER_ERROR_500, null, HttpFields.build().put(HttpHeader.CONNECTION, HttpHeaderValue.CLOSE), 0);
	...
	private static final Index<Boolean> ASSUMED_CONTENT_METHODS = new Index.Builder<Boolean>().caseSensitive(false)
			.with(HttpMethod.POST.asString(), Boolean.TRUE).with(HttpMethod.PUT.asString(), Boolean.TRUE).build();
	...
	private void generateHeaders(ByteBuffer header, ByteBuffer content, boolean last) {
				...
				if (h == null)
					...
					case CONTENT_LENGTH:
						if (contentLength < 0)
							...
							throw new BadMessageException(INTERNAL_SERVER_ERROR_500, String
									.format("Incorrect Content-Length %d!=%d", contentLength, field.getLongValue()));
					...
					case CONNECTION: {
						...
						if (keepAlive && _persistent == Boolean.FALSE) {
							field = new HttpField(HttpHeader.CONNECTION, Stream.of(field.getValues())
									.filter(s -> !HttpHeaderValue.KEEP_ALIVE.is(s)).collect(Collectors.joining(", ")));
						}
						putTo(field, header);
						break;
					}
		...
		// Calculate how to end _content and connection, _content length and transfer
		// encoding
		...
		// persistent
		// or it is a request with content (which cannot EOF) or the app has requested
		// chunking
		...
		// Else if we known the content length and are a request or a persistent
		// response,
	...
	private static final byte[][] SEND = new byte[][] { new byte[0], StringUtil.getBytes("Server: Jetty(10.x.x)\r\n"),
