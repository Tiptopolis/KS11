package com.uchump.prime.Metatron.Lib._HTTP._Jetty.jmx;
...
class MetaData {
	...
	private static Object newInstance(Constructor<?> constructor, Object bean) {
		try {
			Object mbean = constructor.getParameterCount() == 0 ? constructor.newInstance()
	...
	private void collectMBeanAttributeInfos(Map<String, MBeanAttributeInfo> attributeInfos) {
		// Start with interfaces, overwrite with superClass, then overwrite with local
		// attributes.
		for (MetaData intf : _interfaces) {
	...
	private void collectMBeanOperationInfos(Map<String, MBeanOperationInfo> operationInfos) {
		...
		if (_parent != null) {
			...
			for (MBeanOperationInfo parentOperation : parentOperations) {
				String signature = signature(parentOperation.getName(), Arrays.stream(parentOperation.getSignature())
						.map(MBeanParameterInfo::getType).toArray(String[]::new));
	...
	public String toString() {
		return String.format("%s@%x[%s, attrs=%s, opers=%s]", getClass().getSimpleName(), hashCode(), _klass.getName(),
				_attributes.keySet(), _operations.keySet());
	}
	...
	private static class AttributeInfo {
		...
		private AttributeInfo(ManagedAttribute attribute, Method getter) {
			...
			_info = new MBeanAttributeInfo(name, signature, description, true, _setter != null,
	...
	private static class OperationInfo {
		...
		private OperationInfo(ManagedOperation operation, Method method) {
			...
			MBeanParameterInfo[] parameterInfos = parameters(method.getParameterTypes(),
		...
		private static MBeanParameterInfo[] parameters(Class<?>[] parameterTypes,
		...
		@Override
		public String toString() {
			return String.format("%s@%x[%s,proxied=%b,convert=%b]", getClass().getSimpleName(), hashCode(), _name,
					_proxied, _convert);
		}
	}
}