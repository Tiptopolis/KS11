...
public class MimeTypes {
	...
	public enum Type {
		FORM_ENCODED("application/x-www-form-urlencoded"), MESSAGE_HTTP("message/http"),
		MULTIPART_BYTERANGES("multipart/byteranges"), MULTIPART_FORM_DATA("multipart/form-data"),

		TEXT_HTML("text/html"), TEXT_PLAIN("text/plain"), TEXT_XML("text/xml"),
		TEXT_JSON("text/json", StandardCharsets.UTF_8), APPLICATION_JSON("application/json", StandardCharsets.UTF_8),
	...
	public static final Index<Type> CACHE = new Index.Builder<Type>().caseSensitive(false).withAll(() -> {
		...
		try (InputStream stream = MimeTypes.class.getResourceAsStream(resourceName)) {
			if (stream == null) {
				...
				try (InputStreamReader reader = new InputStreamReader(stream, StandardCharsets.UTF_8)) {
					...
					props.load(reader);
					props.stringPropertyNames().stream().filter(x -> x != null).forEach(x -> __dftMimeMap
							.put(StringUtil.asciiToLowerCase(x), normalizeMimeType(props.getProperty(x))));
	...
	/**
	 * Get the MIME type by filename extension. Lookup only the static default mime
	 * map.
	 *
	...
	/**
	 * Get the MIME type by filename extension. Lookup the content and static
	 * default mime maps.
	 *
	 * @param filename A file name
	 * @return MIME type matching the longest dot extension of the file name.
	...
	/**
	 * Access a mutable map of mime type to the charset inferred from that content
	 * type. An inferred encoding is used by when encoding/decoding a stream and is
	...
	/**
	 * Access a mutable map of mime type to the charset assumed for that content
	 * type. An assumed encoding is used by when encoding/decoding a stream, but is
	 * not explicitly set in any metadata (eg Content-Type).
	 *
	 * @return Map of mime type to charset
	 */
