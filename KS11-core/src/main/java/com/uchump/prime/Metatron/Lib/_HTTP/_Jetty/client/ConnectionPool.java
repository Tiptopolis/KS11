...
public interface ConnectionPool extends Closeable {
	/**
	 * Optionally pre-create up to {@code connectionCount} connections so they are
	...
	/**
	 * <p>
	 * Returns an idle connection, if available; if an idle connection is not
	 * available, and the given {@code create} parameter is {@code true}, then
	 * schedules the opening of a new connection, if possible within the
	 * configuration of this connection pool (for example, if it does not exceed the
	 * max connection count); otherwise returns {@code null}.
	 * </p>
	 *
	 * @param create whether to schedule the opening of a connection if no idle
	 *               connections are available
	 * @return an idle connection or {@code null} if no idle connections are
	 *         available
	 */
	 ...
	 * {@link #acquire(boolean)}, back to this ConnectionPool.
	 * </p>
	 *
	...
	interface Multiplexable {
		...
		/**
		 * @param maxMultiplex the max number of requests multiplexable on a single
		 *                     connection
		 * @deprecated do not use, as the maxMultiplex value is pulled, rather than
		 *             pushed
		 */
		@Deprecated
