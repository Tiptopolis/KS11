...
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.security.LoggedOutAuthentication;
...
public class DeferredAuthentication implements Authentication.Deferred {
	...
	public Authentication authenticate(ServletRequest request) {
		try {
			...
			if (authentication != null && (authentication instanceof Authentication.User)
					&& !(authentication instanceof Authentication.ResponseSent)) {
				...
				if (identityService != null)
					_previousAssociation = identityService
							.associate(((Authentication.User) authentication).getUserIdentity());
	 ...
	 * @return true if this response is from a deferred call to
	 *         {@link #authenticate(ServletRequest)}
	 */
