...
public class HttpProxy extends ProxyConfiguration.Proxy {
	...
	private HttpProxy(Origin.Address address, boolean secure, SslContextFactory.Client sslContextFactory,
	...
	private class HttpProxyClientConnectionFactory implements ClientConnectionFactory {
		...
		@Override
		public com.uchump.prime.Metatron.Lib._HTTP._Jetty.io.Connection newConnection(EndPoint endPoint,
		...
		private boolean proxySpeaksServerProtocol(Origin.Protocol serverProtocol) {
			return serverProtocol != null && getProtocol().getProtocols().stream()
					.anyMatch(p -> serverProtocol.getProtocols().stream().anyMatch(p::equalsIgnoreCase));
		}

		private com.uchump.prime.Metatron.Lib._HTTP._Jetty.io.Connection newProxyConnection(EndPoint endPoint, Map<String, Object> context)
				throws IOException {
			// Replace the promise with the proxy promise that creates the tunnel to the
			// server.
			@SuppressWarnings("unchecked")
			Promise<Connection> promise = (Promise<Connection>) context
					.get(HttpClientTransport.HTTP_CONNECTION_PROMISE_CONTEXT_KEY);
	 ...
	 * </p>
	 * <p>
	 * It is implemented as a promise because it needs to establish the tunnel after
	 * the TCP connection is succeeded, and needs to notify the nested promise when
	 * the tunnel is established (or failed).
	 * </p>
	 */
	private static class CreateTunnelPromise implements Promise<Connection> {
		...
		private CreateTunnelPromise(ClientConnectionFactory connectionFactory, EndPoint endPoint,
				Promise<Connection> promise, Map<String, Object> context) {
			this.connectionFactory = connectionFactory;
		...
		private void tunnel(HttpDestination destination, Connection connection) {
			...
			Request connect = new TunnelRequest(httpClient, proxyAddress).method(HttpMethod.CONNECT).path(target)
		...
		private void tunnelSucceeded(EndPoint endPoint) {
			try {
				...
				if (destination.isSecure()) {
					...
					InetSocketAddress address = InetSocketAddress.createUnresolved(destination.getHost(),
							destination.getPort());
			...
			public void onHeaders(Response response) {
				...
				if (response.getStatus() == HttpStatus.OK_200) {
					...
					HttpResponseException failure = new HttpResponseException(
							"Unexpected " + response + " for " + response.getRequest(), response);
