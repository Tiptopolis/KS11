...
/**
 * <p>
 * A ClientConnectionFactory that creates client-side {@link SslConnection}
 * instances.
 * </p>
 */
public class SslClientConnectionFactory implements ClientConnectionFactory {
	...
	public SslClientConnectionFactory(SslContextFactory sslContextFactory, ByteBufferPool byteBufferPool,
			Executor executor, ClientConnectionFactory connectionFactory) {
	...
	/**
	 * @param requireCloseMessage whether peers must send the TLS
	 *                            {@code close_notify} message
	...
	protected SslConnection newSslConnection(ByteBufferPool byteBufferPool, Executor executor, EndPoint endPoint,
			SSLEngine engine) {
		return new SslConnection(byteBufferPool, executor, endPoint, engine, isDirectBuffersForEncryption(),
	 ...
	 * </p>
	 * <p>
	 * Typically implemented by {@link SslContextFactory.Client} to support more
	 * flexible creation of SSLEngine instances.
	 * </p>
	 */
			...
			if (verifier != null) {
				...
				if (address instanceof InetSocketAddress) {
					...
					try {
						...
						throw (SSLException) new SSLPeerUnverifiedException(
								"Host name verification failed for host: " + host).initCause(x);
					}
				}
			}
		}
