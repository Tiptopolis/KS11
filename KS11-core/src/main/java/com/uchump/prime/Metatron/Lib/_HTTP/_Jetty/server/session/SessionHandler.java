...
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.http.HttpCookie;
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.http.Syntax;
...
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.server.SessionIdManager;
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.server.handler.ContextHandler;
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.server.handler.ScopedHandler;
...
import com.uchump.prime.Metatron.Lib._HTTP._Jetty.util.thread.ScheduledExecutorScheduler;
...
public class SessionHandler extends ScopedHandler {
	...
	/**
	 * Session cookie name. Defaults to <code>JSESSIONID</code>, but can be set with
	 * the <code>org.eclipse.jetty.servlet.SessionCookie</code> context init
	 * parameter.
	 */
	...
	/**
	 * Session id path parameter name. Defaults to <code>jsessionid</code>, but can
	 * be set with the
	 * <code>org.eclipse.jetty.servlet.SessionIdPathParameterName</code> context
	 * init parameter. If context init param is "none", or
	 * setSessionIdPathParameterName is called with null or "none", no URL rewriting
	 * will be done.
	 */
	...
	/**
	 * Session Domain. If this property is set as a ServletContext InitParam, then
	 * it is used as the domain for session cookies. If it is not set, then no
	 * domain is specified for the session cookie.
	 */
	...
	/**
	 * Session Path. If this property is set as a ServletContext InitParam, then it
	 * is used as the path for the session cookie. If it is not set, then the
	...
	/**
	 * Session Max Age. If this property is set as a ServletContext InitParam, then
	 * it is used as the max age for the session cookie. If it is not set, then a
	 * max age of -1 is used.
	 */
	public static final String __MaxAgeProperty = "org.eclipse.jetty.servlet.MaxAge";

	public static final Set<SessionTrackingMode> DEFAULT_SESSION_TRACKING_MODES = Collections
			.unmodifiableSet(new HashSet<>(Arrays.asList(SessionTrackingMode.COOKIE, SessionTrackingMode.URL)));

	@SuppressWarnings("unchecked")
	public static final Class<? extends EventListener>[] SESSION_LISTENER_TYPES = new Class[] {
			HttpSessionAttributeListener.class, HttpSessionIdListener.class, HttpSessionListener.class };
	...
	/**
	 * Called by the {@link SessionHandler} when a session is first accessed by a
	 * request.
	 *
	 * Updates the last access time for the session and generates a fresh cookie if
	 * necessary.
	 *
	 * @param session the session object
	 * @param secure  whether the request is secure or not
	 * @return the session cookie. If not null, this cookie should be set on the
	 *         response to either migrate the session or to refresh a session cookie
	 *         that may expire.
	...
	public HttpCookie access(HttpSession session, boolean secure) {
		...
		if (s.access(now)) {
			...
			if (isUsingCookies() && (s.isIdChanged() || (getSessionCookieConfig().getMaxAge() > 0
					&& getRefreshCookieAge() > 0 && ((now - s.getCookieSetTime()) / 1000 > getRefreshCookieAge())))) {
				HttpCookie cookie = getSessionCookie(session, _context == null ? "/" : (_context.getContextPath()),
						secure);
				s.cookieSet();
	 ...
	 *
	 * @param listener the session event listener to add Individual SessionManagers
	 *                 implementations may accept arbitrary listener types, but they
	...
	/**
	 * Called when a response is about to be committed. We might take this
	 * opportunity to persist the session so that any subsequent requests to other
	 * servers will see the modifications.
	 */
	...
	protected void doStart() throws Exception {
		...
		synchronized (server) {
			// Get a SessionDataStore and a SessionDataStore, falling back to in-memory
			// sessions only
	 ...
	 * @return the <code>HttpSession</code> with the corresponding id or null if no
	...
	/**
	 * @return the max period of inactivity, after which the session is invalidated,
	 *         in seconds.
	...
	/**
	 * @return same as SessionCookieConfig.getSecure(). If true, session cookies are
	 *         ALWAYS marked as secure. If false, a session cookie is ONLY marked as
	 *         secure if _secureRequestOnly == true and it is an HTTPS request.
	 */
	...
	/**
	 * @return true if session cookie is to be marked as secure only on HTTPS
	 *         requests
	 */
	...
	/**
	 * HTTPS request. Can be overridden by setting
	 * SessionCookieConfig.setSecure(true), in which case the session cookie will be
	 * marked as secure on both HTTPS and HTTP.
	 *
	...
	/**
	 * A session cookie is marked as secure IFF any of the following conditions are
	 * true:
	 * <ol>
	 * <li>SessionCookieConfig.setSecure == true</li>
	 * <li>SessionCookieConfig.setSecure == false &amp;&amp;
	 * _secureRequestOnly==true &amp;&amp; request is HTTPS</li>
	 * </ol>
	 * According to SessionCookieConfig javadoc, case 1 can be used when: "... even
	 * though the request that initiated the session came over HTTP, is to support a
	 * topology where the web container is front-ended by an SSL offloading load
	 * balancer. In this case, the traffic between the client and the load balancer
	 * will be over HTTPS, whereas the traffic between the load balancer and the web
	 * container will be over HTTP."
	 * <p>
	 ...
	 * which is: <cite> "they shall be marked as secure only if the request that
	 * initiated the corresponding session was also secure" </cite>
	 * <p>
	 * The default for _secureRequestOnly is true, which gives the above behavior.
	 * If you set it to false, then a session cookie is NEVER marked as secure, even
	 * if the initiating request was secure.
	 *
	 * @param session         the session to which the cookie should refer.
	 * @param contextPath     the context to which the cookie should be linked. The
	 *                        client will only send the cookie value when requesting
	 *                        resources under this path.
	 * @param requestIsSecure whether the client is accessing the server over a
	 *                        secure protocol (i.e. HTTPS).
	 * @return if this <code>SessionManager</code> uses cookies, then this method
	 *         will return a new {@link Cookie cookie object} that should be set on
	 *         the client in order to link future HTTP requests with the
	 *         <code>session</code>. If cookies are not in use, this method returns
	 *         <code>null</code>.
	 */
	public HttpCookie getSessionCookie(HttpSession session, String contextPath, boolean requestIsSecure) {
		if (isUsingCookies()) {
			...
			cookie = new HttpCookie(getSessionCookieName(_cookieConfig), id, cookieConfig.getDomain(), sessionPath,
	...
	/**
	 * @return the URL path parameter name for session id URL rewriting, by default
	 *         "jsessionid".
	...
	/**
	 * @return a formatted version of {@link #getSessionIdPathParameterName()}, by
	 *         default ";" + sessionIdParameterName + "=", for easier lookup in URL
	 *         strings.
	...
	/**
	 * @param session the session object
	 * @return the unique id of the session within the cluster (without a node id
	 *         extension)
	...
	public HttpSession newHttpSession(HttpServletRequest request) {
		...
		Session session = _sessionCache.newSession(request, id, created,
				(_dftMaxIdleSecs > 0 ? _dftMaxIdleSecs * 1000L : -1));
	...
	/**
	 * Set Session cookie sameSite mode. Currently this is encoded in the session
	 * comment until sameSite is supported by {@link SessionCookieConfig}
	 *
	 * @param sameSite The sameSite setting for Session cookies (or null for no
	 *                 sameSite setting)
	 */
	public void setSameSite(HttpCookie.SameSite sameSite) {
		// Encode in comment whilst not supported by SessionConfig, so that it can be
		// set/saved in
	...
	/**
	 * Sets the max period of inactivity, after which the session is invalidated, in
	 * seconds.
	 *
	 ...
	 *
	 * @param param the URL path parameter name for session id URL rewriting (null
	 *              or "none" for no rewriting).
	 * @see #getSessionIdPathParameterName()
	 * @see #getSessionIdPathParameterNamePrefix()
	 */
	public void setSessionIdPathParameterName(String param) {
		_sessionIdPathParameterName = (param == null || "none".equals(param)) ? null : param;
		_sessionIdPathParameterNamePrefix = (param == null || "none".equals(param)) ? null
	 ...
	 * @param invalidate True if
	 *                   {@link HttpSessionListener#sessionDestroyed(HttpSessionEvent)}
	 *                   and {@link SessionIdManager#expireAll(String)} should be
	 *                   called.
	...
	public Session removeSession(String id, boolean invalidate) {
		try {
			...
			// TODO if session object is not known to this node, how to get rid of it if no
			// other
			// node knows about it?
	...
	public void setSessionTrackingModes(Set<SessionTrackingMode> sessionTrackingModes) {
		if (sessionTrackingModes != null && sessionTrackingModes.size() > 1
	...
	/**
	 * @return True if absolute URLs are check for remoteness before being session
	 *         encoded.
	 */
	...
	/**
	 * Record length of time session has been active. Called when the session is
	 ...
	 * either by this context or another context. Also called by SessionIdManager
	 * when a session has expired in either this context or another context.
	 *
	...
	/**
	 * Called periodically by the HouseKeeper to handle the list of sessions that
	 * have expired since the last call to scavenge.
	 */
	public void scavenge() {
		...
		try {
			...
			LOG.warn("Failed to check expiration on {}",
					candidates.stream().map(Objects::toString).collect(Collectors.joining(", ", "[", "]")), e);
		}
	}

	/**
	 * Each session has a timer that is configured to go off when either the session
	 * has not been accessed for a configurable amount of time, or the session
	 * itself has passed its expiry.
	 *
	 * If it has passed its expiry, then we will mark it for scavenging by next run
	 * of the HouseKeeper; if it has been idle longer than the configured eviction
	 * period, we evict from the cache.
	 *
	 * If none of the above are true, then the System timer is inconsistent and the
	 * caller of this method will need to reset the timer.
	 *
	 ...
	 * SessionIf
	 *
	 * Interface that any session wrapper should implement so that SessionManager
	 * may access the Jetty session implementation.
	 */
	 ...
	 * CookieConfig
	 *
	 * Implementation of the javax.servlet.SessionCookieConfig. SameSite
	 * configuration can be achieved by using setComment
	 *
	 * @see HttpCookie
	 */
	...
	public void doScope(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			if (LOG.isDebugEnabled())
				LOG.debug("Entering scope {}, dispatch={} asyncstarted={}", this, baseRequest.getDispatcherType(),
						baseRequest.isAsyncStarted());
			...
			if ((existingSession != null) && (oldSessionHandler != this)) {
				...
				// Handle changed ID or max-age refresh, but only if this is not a redispatched
				// request
				if ((cookie != null) && (request.getDispatcherType() == DispatcherType.ASYNC
			...
			if (LOG.isDebugEnabled())
				LOG.debug("Leaving scope {} dispatch={}, async={}, session={}, oldsession={}, oldsessionhandler={}",
						this, baseRequest.getDispatcherType(), baseRequest.isAsyncStarted(),
						baseRequest.getSession(false), oldSession, oldSessionHandler);

			// revert the session handler to the previous, unless it was null, in which case
			// remember it as
						...
						if (session == null) {
							...
							if (!session.getId().equals(getSessionIdManager().getId(id))) {
								...
								if (s != null && isValid(s)) {
									...
									if (LOG.isDebugEnabled())
										LOG.debug("Multiple different valid session ids: {}, {}", requestedSessionId,
												id);
									throw new BadMessageException(
											"Duplicate valid session cookies: " + requestedSessionId + " ," + id);
								}
							} else {
								if (LOG.isDebugEnabled())
