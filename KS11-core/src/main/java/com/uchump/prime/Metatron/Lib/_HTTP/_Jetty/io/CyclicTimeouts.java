...
/**
 * <p>
 * An implementation of a timeout that manages many {@link Expirable expirable}
 * entities whose timeouts are mostly cancelled or re-scheduled.
 * </p>
 * <p>
 * A typical scenario is for a parent entity to manage the timeouts of many
 * children entities.
 * </p>
 * <p>
 * When a new entity is created, call {@link #schedule(Expirable)} with the new
 * entity so that this instance can be aware and manage the timeout of the new
 * entity.
 * </p>
 * <p>
 * Eventually, this instance wakes up and iterates over the entities provided by
 * {@link #iterator()}. During the iteration, each entity:
 * </p>
 * <ul>
 * <li>may never expire (see {@link Expirable#getExpireNanoTime()}; the entity
 * is ignored</li>
 * <li>may be expired; {@link #onExpired(Expirable)} is called with that entity
 * as parameter</li>
 * <li>may expire at a future time; the iteration records the earliest
 * expiration time among all non-expired entities</li>
 * </ul>
 * <p>
 * When the iteration is complete, this instance is re-scheduled with the
 * earliest expiration time calculated during the iteration.
 * </p>
 *
...
public abstract class CyclicTimeouts<T extends CyclicTimeouts.Expirable> implements Destroyable {
	 ...
	 * </p>
	 * <p>
	 * This method may be invoked multiple times, and even concurrently, for the
	 * same expirable entity and therefore the expiration of the entity, if any,
	 * should be an idempotent action.
	 * </p>
	 *
	 * @param expirable the entity that is expired
	 * @return whether the entity should be removed from the iterator via
	...
	private void onTimeoutExpired() {
		...
		while (iterator.hasNext()) {
			...
			if (LOG.isDebugEnabled())
				LOG.debug("Entity {} expires in {} ms for {}", expirable,
						TimeUnit.NANOSECONDS.toMillis(expiresAt - now), this);
	...
	private void schedule(long expiresAt) {
		...
		while (expires < prevEarliest) {
			...
			// If we lost a race and overwrote a schedule() with an earlier time, then that
			// earlier time
			// is remembered by earliestTimeout, in which case we will loop and set it again
			// ourselves.
	...
	public interface Expirable {
		 ...
		 * {@link System#nanoTime()}, for example:
		 * </p>
		 ...
		 * </p>
		 *
		 * @return the expiration time in nanoseconds, or {@link Long#MAX_VALUE} if this
