...
import com.uchump.prime.Core.Primitive.Struct.aDictionary.D_Key;
import com.uchump.prime.Core.Utils.StringUtils;

public class bDictionary<V> extends aMultiMap<Entry<Object, String>, V> {
	public void put(B_Key key, Object val) {
		if (!this.contains(key, val)) {
			if (this.containsKey(key))
				key = (bDictionary<V>.B_Key) this.keys.get(this.keys.indexOf(key));
	...
	public void put(Entry<Object, String> key, Object val) {
		if (!this.contains(key, val)) {
			if (this.containsKey(key))
				key = (B_Key) this.keys.get(this.keys.indexOf(key));
			this.keys.append((B_Key) key);
			this.values.append((V) val);
		}
	}
	...
	public void put(Object context, String label, V as) {
		...
		B_Key D = newEntry(context, label);
	...
	@Override
	public B_Key newEntry(Object key, Object val) {
		return new B_Key(key, "" + val);
	}

	public class B_Key extends _Map.Entry<Object, String> {

		public B_Key(Object k, String v) {
			super(k, v);
		}
